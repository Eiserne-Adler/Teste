...
FuelSystem2::FuelSystem2( Scooter::Engine2& engine, Engine1& engine1b, Scooter::AircraftState& state ) :
	m_engine( engine ),
	m_engine1( engine1b ),
	m_state( state )
{
...
void FuelSystem2::update( double dt )
{
	...
	double dm = (m_engine.getFuelFlow() + m_engine1.getFuelFlow()) * dt;  //
	//double dm = m_engine1.getFuelFlow() * dt; //

	double enginePower = m_engine.getRPMNorm() > 0.40; //
	double enginePower1 = m_engine1.getRPMNorm() > 0.40; //
	...
	if ( m_boostPump )
	{
		...
		m_engine1.setMaxDeliverableFuelFlowFraction(1.0); //
	...
	else
	{
		...
		m_engine1.setMaxDeliverableFuelFlowFraction(maxDeliverable); //
		//printf( "g: %lf\n", m_state.getGForce() );
	...
	m_engine1.setHasFuel(m_hasFuel); //
	...
	if (m_externalTankPressure && enginePower1)
	{
		double externWingRate = rateFactor * c_fuelTransferRateExternWingToWing * dt;
		double externCentreRate = rateFactor * c_fuelTransferRateExternCentreToWing * dt;
