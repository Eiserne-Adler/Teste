...
public:
	...
	FuelSystem2( Scooter::Engine2& engine, Scooter::Engine1& engine1b, Scooter::AircraftState& state );
...
void FuelSystem2::updateMechPumpPressure()
{
	...
	m_mechPumpPressure = 4.0 * (m_engine.getRPMNorm() * m_engine.getRPMNorm() + m_engine1.getRPMNorm() * m_engine1.getRPMNorm()); // REVER FORMULA
}

double FuelSystem2::transferRateFactor()
{
	return clamp( 2.0 * (m_engine.getRPMNorm() * m_engine.getRPMNorm() + m_engine.getRPMNorm() * m_engine.getRPMNorm()), 0.0, 1.0 ); // REVER FORMULA
}
