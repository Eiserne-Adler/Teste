...
static Scooter::Engine1* s_engine1 = NULL; //
...
void init(const char* config)
{
	...
	s_engine1 = new Scooter::Engine1( *s_state); //
	s_airframe = new Scooter::Airframe( *s_state, *s_input, *s_engine, *s_engine1 ); //
	s_avionics = new Scooter::Avionics( *s_input, *s_state, *s_interface );
	s_fm = new Scooter::FlightModel( *s_state, *s_input, *s_airframe, *s_engine, *s_engine1, *s_interface, s_splines ); //
	s_ils = new Scooter::ILS(*s_interface);
	s_fuelSystem = new Scooter::FuelSystem2( *s_engine, *s_engine1, *s_state ); //
...
void ed_fm_simulate(double dt)
{
	...
	if ( s_interface->getAvionicsAlive() )
	{
		...
		s_engine1->setThrottle1(s_interface->getEngineThrottlePosition1()); //
		s_engine1->setBleedAir1(s_interface->getBleedAir1() > 0.1); //
		s_engine1->setIgnitors1(s_interface->getIgnition1() > 0.1); //
	...
	s_interface->setRPM(s_engine1->getRPMNorm() * 100.0); //
	s_interface->setFuelFlow(s_engine1->getFuelFlow()); //

	s_interface->setThrottlePosition(s_input->throttleNorm()); //
	s_interface->setThrottlePosition1(s_input->throttleNorm()); //
...
)
{
	...
	if ( s_engine->handleInput( command, value ) ) //
		return; //
	if (s_engine1->handleInput(command, value)) //
		return; //
}
...
double ed_fm_get_param(unsigned index)
{
	...
	case ED_FM_ENGINE_1_FUEL_FLOW:
		return s_engine1->getFuelFlow();

	case ED_FM_ENGINE_1_RELATED_THRUST:
	case ED_FM_ENGINE_1_CORE_RELATED_THRUST:
		return s_engine1->getThrust() / c_maxStaticThrust;
	case ED_FM_ENGINE_1_RELATED_RPM:
		return s_engine1->getRPMNorm();
	...
	case ED_FM_ENGINE_1_THRUST:
		return s_engine1->getThrust();
	case ED_FM_ENGINE_1_COMBUSTION:
		return s_engine1->getFuelFlow() / c_fuelFlowMax;
...
bool ed_fm_pop_simulation_event(ed_fm_simulation_event& out)
{
	...
	else if (s_airframe->catapultState() == Scooter::Airframe::ON_CAT_READY)
	{
		...
		if ( ! autoMode )
		{
			...
			thrust = s_engine->getThrust() + s_engine1->getThrust();
		}
